...
def suite(global_refs,local_refs,comment):
  ...
  try:
    ...
    print("[SUITE] PageDisplayList="+str(MyArbo.PageDisplayList)+" MyNbPage="+str(MyNbPage)) 
...
def retour(global_refs,local_refs,comment):
  ...
  try:
    ...
    print("[RETOUR] PageDisplayList="+str(MyArbo.PageDisplayList)+" MyNbPage="+str(MyNbPage)) 

    if MyArbo.DoSendPage==False:                  # Si le traitement préalable de RETOUR n'a pas provoqué l'affichage d'une nouvelle page (cas d'une séquence de pages)
      if MyArbo.RefreshCurField==False:           # Si le traitement préalable de RETOUR n'a pas provoqué l'affichage d'un nouveau champ
        if MyArbo.PageDisplayList >=0 :           # Si on n'était pas déjà sur la première page de la liste
          if MyNbPage>0:                          # Si on a des pages dans la liste
            MyArbo.DebugInput += "$WasNotFirstPageOfList$"
            MyArbo.DoSendPage=True
            Success=True
          else:
            MyArbo.DebugInput += "$NoPageInList$"
        else:
          MyArbo.DebugInput += "$FirstPageOfList$"
      else:
        MyArbo.DebugInput += "$NextFieldDetected$"
    if False:
      Success=True
